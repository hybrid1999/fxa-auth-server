...
async function create (log, error, config, routes, db, oauthdb, translator) {
  ...
  // we will end up with refreshTokenId entries in devices table where sessionTokenId is NULL.

  // There is some other change for sessionTokens, follow up PRs???? Where
          // for Fenix, devices & apps list --> current device????
          //                                --> signing out??
          //                                --> migration??
          // sessionTokenIds will start populating in a follow up, where during the code exchange, we also db.createSessionToken().
          // it's ok if devices table has NULLs.

          // the /token PR (code exchange) also should create a placeholder device??

          // get sessionTokenId from migration,
          // Create a mock sessionToken that can be used in functions expect one.

          // follow up PRs -> get rid of the /clients-tokens PR.
          //

          // Part 7. Fenix migration, what's up with their device registrations.
            // Pop the refreshTokenId on the new column, into existing entry.
           // The app needs to reset its device record:

            // Remove the push subscriptions???
            // Fenix would push its own endpoints
          //

          const sessionToken = {
            uid: refreshToken.sub,
            id: refreshTokenId, // XXX: get rid of this
